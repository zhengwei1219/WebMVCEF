...
(function ($) {
    ...
    $.extend($.jgrid, {
		...
		_useProperties = true;
                ...
                this._compare = function (a, b, d) {
                    ...
                    if (toString.call(a) === '[object Number]' && toString.call(b) === '[object Number]') {
                        if (a < b) { return -d; }
                        if (a > b) { return d; }
                        return 0;
                    }
                    ...
                    return a.localeCompare(b) < 0 ? -d : a.localeCompare(b) > 0 ? d : 0;
    ...
    $.fn.jqGrid = function (pin) {
		...
		refreshIndex = function () {
		    ...
		    //重新设置下拉框中的all的长度值为数据数组长度
		    $(".ui-pg-selbox  option:contains('all')").val(datalen); ts.p.page = 1;
		...
		setPager = function (pgid, tp) {
		    ...
		    $('.ui-pg-selbox', "#" + pgcnt).bind('change', function () { 
		        ...
		        if (this.value == 'all') {//如果为all，表示取所有的，页码重值为1
		            ts.p.page = 1;
		            ts.p.rowNum = ts.p.data.length;
		            if (ts.p.pager) { $('.ui-pg-selbox', ts.p.pager).val(ts.p.data.length); }
		            if (ts.p.toppager) { $('.ui-pg-selbox', ts.p.toppager).val(ts.p.data.length); }
		        }
		        else { 
                    ...
                    try {
                        $(this.p.colModel).each(function (i) {
                            ...
                            if (data[nm] !== undefined) {
                                ...
                                vl = t.formatter(rowid, data[nm], i, data, 'edit'); alert(title); alert($.jgrid.stripHtml(vl));
...
"use strict";	
    ...
    // html tag to lower
	function TagToLower(htmlStr) {
	    var ret = htmlStr.toString().replace(/(<\/?)([a-z\d\:]+)((\s+.+?)?>)/gi, function (s, a, b, c) { return a + b.toLowerCase() + c; })
	    return ret;
	}  
	$.unformat.select = function (cellval,options,pos,cnt) {
        ...
        //add method for html option
	    var cell = $(cellval).html();
		cell =TagToLower(cell);
				...
				if(options.dataUrl !== undefined) {
					...
					if(typeof options.value === 'string') {
						...
						for(i=0; i<so.length;i++){
							...
							ov.value = sv[0]; ov.innerHTML = sv[1];
							elem.appendChild(ov);
							if ((!msl &&  ($.trim(sv[0]) == $.trim(vl) || $.trim(sv[1]) == $.trim(vl)))&&$.trim(sv[0])!="") { ov.selected ="selected"; }
...
// Date Validation Javascript
	checkDate : function (format, date) {
		...
		daysArray = function (n) {
			...
			for ( var i = 1; i <= n; i++) {
			    daysarr[i] = 31;
			    if (i == 4 || i == 6 || i == 9 || i == 11) { daysarr[i] = 30; }
				if (i == 2) { daysarr[i] = 29; }
			}
			return daysarr;
		};
